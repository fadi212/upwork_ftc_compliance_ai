...
class LocalIndex(BaseIndex):
    ...
    def query(
        ...
        if route_filter is not None:
            ...
            filtered_routes = []
            filtered_utterances = []
            for route, vec, utterance in zip(self.routes, self.index, self.utterances):
            ...
            routes = [filtered_routes[i] for i in idx]
            utterances = [filtered_utterances[i] for i in idx]
        else:
            sim = similarity_matrix(vector, self.index)
            scores, idx = top_scores(sim, top_k)
            routes = [self.routes[i] for i in idx]
            utterances = [self.utterances[i] for i in idx]
        return scores, routes, utterances
