...
class RouteLayer:
    ...
    def __call__(
        ...
        route, top_class_scores, utterances = self._retrieve_top_route(vector, route_filter)
    ...
    ) -> Tuple[Optional[Route], List[float], List[str]]:
        """
        Retrieve the top matching route based on the given vector.
        Returns a tuple of the route (if any), the scores of the top class, and the matching utterances.
        """
        results, utterances = self._retrieve(
            xq=np.array(vector), top_k=self.top_k, route_filter=route_filter
        )
        top_class, top_class_scores, utterances = self._semantic_classify(results)
        route = self.check_for_matching_routes(top_class)
        return route, top_class_scores, utterances
    ...
    ) -> Tuple[List[Dict[str, Union[float, str]]], List[str]]:
        """Given a query vector, retrieve the top_k most similar records."""
        # get scores and routes
        scores, routes, utterances = self.index.query(
            vector=xq, top_k=top_k, route_filter=route_filter
        )
        return [{"route": d, "score": s.item(), "utterance": u} for s, d, u in zip(scores, routes, utterances)], utterances
    ...
    def _semantic_classify(self, query_results: List[Dict[str, Union[float, str]]]) -> Tuple[str, List[float], List[str]]:
        ...
        # Get the utterances for the top class
        top_class_utterances = [res["utterance"] for res in query_results if res["route"] == top_class]

        # Return the top class and its associated scores and utterances
        if top_class is not None:
            return str(top_class), scores_by_class.get(top_class, []), top_class_utterances
        else:
    ...
    def group_scores_by_class(
        self, query_results: List[Dict[str, Union[float, str]]]
    ) -> Dict[str, List[float]]:
    ...
    def group_utterances_by_class(
        self, query_results: List[Dict]
    ) -> Dict[str, List[str]]:
        utterances_by_class: Dict[str, List[str]] = {}
        for result in query_results:
            utterance = result["utterance"]
            route = result["route"]
            if route in utterances_by_class:
                utterances_by_class[route].append(utterance)
            else:
                utterances_by_class[route] = [utterance]
        return utterances_by_class
