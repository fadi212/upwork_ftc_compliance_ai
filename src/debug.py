...
import uvicorn
from src.custom_data_processing.custom_file_processing import process_text, count_tokens, get_basis_sentences
from src.custom_data_processing.custom_router_setup import load_category_from_s3, init_router
...
# Global cache to store the router layers
router_cache = {}
cache_lock = Lock()
...
def get_router_layer(category, refresh):
    with cache_lock:  # Ensuring thread-safe access to the cache
        if refresh or category not in router_cache:
            if refresh:
            ...
            router_layer = router_cache[category]
...
async def process_text_api(request_body: TextProcessRequest, request: Request):
    ...
    with get_router_layer(category, refresh) as router_layer:
        ...
        for sentence_data in tagged_sentences:
            sentence_data["basis_sentences"] = get_basis_sentences(router_layer, sentence_data["sentence"])
